-- EPR Team Database Schema Updates
-- Run these commands in your Supabase SQL Editor to add EPR team functionality

-- IMPORTANT: Run this script in TWO PARTS due to PostgreSQL enum limitations
-- PART 1: First run these commands, then commit/execute them

-- 1. Update the user_role enum to include EPR
ALTER TYPE user_role ADD VALUE 'epr';

-- 2. Add EPR-specific columns to service_requests table
ALTER TABLE service_requests 
ADD COLUMN current_epr_status TEXT,
ADD COLUMN epr_timeline JSONB DEFAULT '[]'::jsonb;

-- 3. Create EPR status enum type
...
-- 4. Update the current_epr_status column to use the enum
ALTER TABLE service_requests 
ALTER COLUMN current_epr_status TYPE epr_status 
USING current_epr_status::epr_status;

-- 5. Add EPR team user to app_users table
-- NOTE: This must be run AFTER the enum value is committed
-- Run this in a separate transaction after PART 1 is completed
...
CREATE INDEX idx_service_requests_epr_status ON service_requests(current_epr_status);
CREATE INDEX idx_service_requests_epr_timeline ON service_requests USING GIN(epr_timeline);
