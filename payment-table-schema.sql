-- ===================================================================
-- COMPREHENSIVE PAYMENT TABLE SCHEMA FOR SUPABASE
-- A-1 Fence Services Payment Management System
-- ===================================================================

-- Payment status and method enums already exist in the database
-- Using existing payment_status and payment_method enums
...
DO $$
BEGIN
    -- Only add foreign key constraints if the referenced tables exist
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'quotes') THEN
        ALTER TABLE payments 
        ADD CONSTRAINT fk_payments_quote_id 
        FOREIGN KEY (quote_id) REFERENCES quotes(id) ON DELETE CASCADE;
    END IF;
END $$;

-- Add payment fields to existing tables (if they exist)
DO $$
BEGIN
    -- Add payment fields to service_requests table if it exists
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'service_requests') THEN
        ...
        -- Add foreign key constraint if payments table exists
    ...
    -- Add payment fields to quotes table if it exists
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'quotes') THEN
        ALTER TABLE quotes 
        ...
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'payments') THEN
            ALTER TABLE quotes 
            ADD CONSTRAINT fk_quotes_payment_id 
            FOREIGN KEY (payment_id) REFERENCES payments(id);
        END IF;
    END IF;
END $$;
...
-- Insert sample payment data (optional - for testing)
-- INSERT INTO payments (
--     id, service_request_id, quote_id, customer_id, customer_name, 
--     customer_email, amount, currency, status, receipt_number
-- ) VALUES (
--     'pay_sample_001', 'req-abc-123', 'quote-sample-001', 'customer-demo', 
--     'John Doe', 'john@example.com', 1500.00, 'INR', 'captured', 'RCP000001'
-- );
